//! SVM program helpers for SPL Token-2022, including extension-aware account
//! builders.
//!
//! The program ID, ELF, and program-account helpers are re-exported from
//! [`mollusk_svm_programs_token::token2022`], so this crate is a complete,
//! self-contained entry point for Token-2022 testing.
//!
//! # Example
//!
...
pub use {
    mollusk_svm_programs_token::token2022::{account, add_program, keyed_account, ELF, ID},
...
use {solana_account::Account, solana_rent::Rent};
...
impl MintExtension {
    ...
    fn init(&self, state: &mut StateWithExtensionsMut<Mint>) {
        match self {
            ...
            // Variable-length extensions are sized and initialized by the
            // dedicated path in `create_account_for_mint_with_extensions`,
            // the only caller, and never reach `init`.
...
// Wrap account data in a rent-exempt account owned by the Token-2022 program.
fn rent_exempt_account(data: Vec<u8>) -> Account {
    Account {
