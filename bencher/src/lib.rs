...
///
/// Allows developers to bench test compute unit usage on multiple implementation of their programs.
...
impl<'a> MolluskComputeUnitMatrixBencher<'a> {
    /// Create a new matrix bencher, to which benches and configurations can be added.
    pub fn new() -> Self {
        let mut out_dir = PathBuf::from(std::env::var("CARGO_MANIFEST_DIR").unwrap());
        out_dir.push("benches");
        Self {
